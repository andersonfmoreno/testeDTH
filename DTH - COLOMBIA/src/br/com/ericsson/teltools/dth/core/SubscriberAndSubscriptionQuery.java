...
public class SubscriberAndSubscriptionQuery extends JavaFormatterBase implements JavaFormatterInterface
{    
    private static final String INSERT_SUBSCRIPTIONS = "INSERT_SUBSCRIPTIONS";
    private static final String INSERT_SUBSCRIBER = "INSERT_SUBSCRIBER";
    private static final String NUM_STR_DELIMETER_RESULT = "<#>";
    private static final String NUM_STR_DELIMETER = "<\\#>";
    ...
    static final long ERROR_CODE = 10801;
    static final String ERROR_TYPE = "internal";

    static String EMPTY = "";

    static String TARGET = "TARGET";
    static String TARGET_SEARCH_DB = "DSMNDBSearch";
    static String TARGET_CONTROLLER = "QueryResponse";
    ...
    public void format(DataObject dataSource) throws Exception
    {
        try
        {
            ...
            //First check if exist a Action with name CLEAR_ALL, if it exist so clear all arrays.
            if (dataSource.parameterExists(ACTION) && dataSource.getValueAsString(ACTION).equals(CLEAR_ALL))
            {
                clearArrays();
                return;
            }
            if (dataSource.parameterExists(ACTION) && dataSource.parameterExists(POSITION) && dataSource.parameterExists(MAINID) && dataSource.parameterExists(NUMBER_VALUE) && dataSource.parameterExists(TEXT_VALUE))
            {
                ByteBuffer buffer = BufferFactory.getInstance(dataSource.getValueAsString(ACTION).toUpperCase());
                
                if (dataSource.getValueAsString(ACTION).equals(INSERT_SUBSCRIPTIONS))
                {
                    dataSource.setParameter("TEXT_VALUE",signalPositivetoSignalAux(dataSource.getValueAsString("TEXT_VALUE")).toString());
                }
        ...
        {
            dataSource.setParameter("status", ERROR_STATUS);
            dataSource.setParameter("errorSource", ERROR_SOURCE);
            dataSource.setParameter("errorCode", ERROR_CODE);
            dataSource.setParameter("errorType", ERROR_TYPE);
            dataSource.setParameter("errorDescription", e.getMessage() + Arrays.toString(e.getStackTrace()));
        }

    }
    
    private StringBuilder signalPositivetoSignalAux(String textValue)
    {
        String[] vetTextValue = textValue.split(RECORD_DELIMETER), vetTimeStamps;
        StringBuilder result = new StringBuilder();
        
        for (int i = 0; i < vetTextValue.length; i++)
        {
            vetTimeStamps =  vetTextValue[i].split(FIELD_DELIMETER);
            vetTextValue[i] = "";
            for (int j = 0; j < vetTimeStamps.length; j++)
            {
                vetTimeStamps[j] =  vetTimeStamps[j].replace("+", "--");
    ...
    private void findByMAINID(DataObject dataSource) throws NumberFormatException, Exception
    {
        ...
        if (resultContainerMap.containsKey(SUBSCRIPTION_CONTAINER_RESULT)){
            queryResultSubscription = (String) resultContainerMap.get(SUBSCRIPTION_CONTAINER_RESULT);
        }
        ...
        if (queryResultSubscription.isEmpty())
        {
            ...
            if (!queryResultSubscription.isEmpty()){
                queryResultSubscription = signalAuxToPositive(queryResultSubscription);
            }
        }
        if (!queryResultSubscriber.isEmpty())
        {
            ...
            dataSource.setParameter(SUBSCRIBER_MEMORY_PARAM, formatToPutInResponse(dataSource.getValueAsLong(QUERY), queryResultSubscriber));
        }
        else
        {
        ...
        if (!queryResultSubscription.isEmpty())
        {
            ...
            dataSource.setParameter(SUBSCRIPTIONS_MEMORY_PARAM, formatToPutInResponse(dataSource.getValueAsLong(QUERY), queryResultSubscription));
        }
        else
        {
    ...
    private String formatToPutInResponse(long mainID, String string) {
        return (mainID+"<:>"+string).replace("<;>.<;>", "<;> <;>");
    }

    private String signalAuxToPositive(String queryResultSubscription) throws ParseException
    {
        String[] vetData = queryResultSubscription.split(NUM_STR_DELIMETER);
        ...
        return  numberValue + NUM_STR_DELIMETER_RESULT + result.toString();
    }
    ...
    private Map<String, Object> findByMAINIDInContainer(String mainID) throws Exception
    {
        ...
        DataObject subscriptionContainerMap = getSubscriptionContainerBy(mainID);
        DataObject subscriberContainerMap = getSubscriberContainerBy(mainID);
        
        Map<String, Object> resultContainerMap = new java.util.concurrent.ConcurrentHashMap<String, Object>();
        
        if (subscriptionContainerMap != null) {
            resultContainerMap.put(SUBSCRIPTION_CONTAINER_RESULT, subscriptionContainerMap.getValueAsString("D"));
        }
        if (subscriberContainerMap != null) {
            resultContainerMap.put(SUBSCRIBER_CONTAINER_RESULT, subscriberContainerMap.getValueAsString("D"));
        }

        return resultContainerMap;
    }

    private DataObject getSubscriptionContainerBy(String mainID) throws EntityException{
        DataObject result = null;  
        try {
            result = getSubscriptionContainerEntity("SP."+mainID);
        } catch (Exception e) {
            result = null;
        }
        return result; 
    ...
    private DataObject getSubscriberContainerBy(String mainID) throws EntityException{
        DataObject result = null;  
        try {
            result = getSubscriberContainerEntity("SB."+mainID);
        } catch (Exception e) {
            result = null;
        }
        return result; 
    ...
    private DataObject getSubscriberContainerEntity(String entityName) throws EntityException {
        return getEntity(entityName).getData();
    }
    
    private DataObject getSubscriptionContainerEntity(String entityName) throws EntityException {
        return getEntity(entityName).getData();
    }
    ...
    static class ByteBuffer
    {
        ...
        private String query(long mainID) throws Exception
        {
            ...
            String toReturn = numberValue + NUM_STR_DELIMETER_RESULT + textValue;
    ...
    static class BufferFactory
    {
        ...
        {
            if (INSERT_SUBSCRIBER.equals(bufferName))
            {
                return subscribersBuffer;
            }
            if (INSERT_SUBSCRIPTIONS.equals(bufferName))
            {
